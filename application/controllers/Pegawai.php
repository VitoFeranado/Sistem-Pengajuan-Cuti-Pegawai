...
class Pegawai extends CI_Controller {
	...
	public function pengajuan_simpan()
	{
		if(isset($_POST) ){
			...
			$query_bagian = $this->db->query("SELECT id_bagian FROM tb_pegawai WHERE idpeg='$idpeg'");
			$idbagian = $query_bagian->row()->id_bagian;
			$sekarang = date('Y-m-d');
			$query_cek = $this->db->query("select count(*) as jmlcek from vdetail_cuti where tgl_pengajuan='$sekarang' and id_bagian='$idbagian'");
			$cekjumlah = $query_cek->row()->jmlcek;
			if($cekjumlah>=2)
			{
				echo "<div class='alert alert-warning fade in'>
                        <a href='#' class='close' data-dismiss='alert'>&times;</a>
                        <strong>INFO!</strong> Tidak dapat mengajukan cuti, dikarnakan hari ini 2 pegawai telah mengajukan cuti!
                    </div>";
			}
			else
			{
          ...
          if($cek_cutinya->num_rows()>0){
            ...
            if($bat<>0){
              if($jmlnya>$bat){
				...
				$tglakhir = date('Y-m-d',strtotime($selesai));
				
				$rubah1 = explode("-",$tglakhir);
				$tanggal_1 = $rubah1[2];
				$bulan_1 = $rubah1[1];
				$tahun_1 = $rubah1[0];
                ...
                $this->db->query("INSERT INTO tb_ambilcuti (id_peg,id_jeniscuti,jml_diambil,tgl_mulaicuti,tgl_akhircuti,alamat_selamacuti) VALUES('$idpeg','$jeniscuti','$selisih','$tgl','$tglakhir','$alamat')  ");
